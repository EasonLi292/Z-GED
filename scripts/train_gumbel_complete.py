"""
Complete training script for Gumbel-Softmax Component Selection.
"""
...
def collate_circuit_batch(batch_list):
    ...
    return {
        'graph': batched_graph,
        'poles': poles,  # List of pole tensors
        'zeros': zeros   # List of zero tensors
    }
...
def graph_to_dense_format(graph, max_nodes=5):
    ...
    for b in range(batch_size):
        ...
        # Node types
        graph_node_types = graph.x[mask].argmax(dim=-1)  # [num_nodes]
...
def train_epoch(encoder, decoder, dataloader, loss_fn, optimizer, device, epoch):
    ...
    for batch in pbar:
        ...
        # Forward pass through encoder (returns tuple: z, mu, logvar)
        z, mu, logvar = encoder(
            graph.x,
            graph.edge_index,
            graph.edge_attr,
        ...
        # Dummy conditions (you can extract from specs if available)
...
def main():
    """Main training loop."""
    print("="*70)
    print("Gumbel-Softmax Component Selection Training")
    print("="*70)
    ...
    print("Loading stratified train/val split...")
    split_data = torch.load('stratified_split.pt')
    train_indices = split_data['train_indices']
    val_indices = split_data['val_indices']

    print(f"Train distribution: {split_data['train_distribution']}")
    print(f"Val distribution: {split_data['val_distribution']}")
    ...
    loss_fn = GumbelSoftmaxCircuitLoss(
        ...
        component_type_weight=10.0,  # INCREASED: Focus on learning component selection
        component_value_weight=0.5,  # DECREASED: Less important initially
        use_connectivity_loss=True,
        connectivity_weight=2.0  # DECREASED: Less important than component type
    )
    ...
    checkpoint_dir = 'checkpoints/gumbel_softmax'
